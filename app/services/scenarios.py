...
def list_scenario_sources(root_dir: str) -> list[Path]:
    root = Path(root_dir)
    if not root.exists() or not root.is_dir():
        return []
    return sorted(
        [p for p in root.iterdir() if p.is_dir() and (p / "manifest.yaml").exists()],
        key=lambda p: p.name.lower(),
    )


def resolve_scenario_source(root_dir: str, scenario_id: str) -> Path:
    if not scenario_id:
